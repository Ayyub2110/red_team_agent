...
class TestTargetValidation:
    ...
    def test_valid_target_in_subnet(self) -> None:
        """Target inside 172.20.0.0/24 should pass."""
        from src.guardrails import validate_target

        # Should not raise
        validate_target("172.20.0.10")
        validate_target("172.20.0.255")
        validate_target("172.20.0.1")

    def test_reject_external_ip(self) -> None:
        """Target outside the subnet must be rejected."""
    ...
    def test_reject_localhost(self) -> None:
        """Localhost must be rejected (not in target subnet)."""
        from src.guardrails import validate_target, TargetValidationError

        with pytest.raises(TargetValidationError, match="OUTSIDE"):
            validate_target("127.0.0.1")

    def test_reject_private_wrong_subnet(self) -> None:
        """Private IPs in wrong subnet must be rejected."""
        from src.guardrails import validate_target, TargetValidationError
...
class TestBlockedCommands:
    ...
    def test_safe_command_allowed(self) -> None:
        ...
        # Should not raise
        check_blocked_commands("nmap -sV 172.20.0.10")
...
class TestConfig:
    ...
    def test_default_settings(self) -> None:
        ...
        assert "172.20.0.0/24" in settings.safety.allowed_target_subnet
...
class TestAuditLogger:
    ...
    def test_audit_record_creates_file(self, tmp_path) -> None:
        ...
        assert entries[0]["target"] == "172.20.0.10"
...
class TestPrompts:
    ...
    def test_system_prompt_renders(self) -> None:
        ...
        assert "172.20.0.0/24" in prompt
